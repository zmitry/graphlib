'use strict';Object.defineProperty(exports,'__esModule',{value:true});var lodash=require('lodash');const DEFAULT_EDGE_NAME = "\x00";
...
class Graph {
  ...
  sources() {
    const self = this;
    return lodash.filter(this.nodes(), (v) => lodash.isEmpty(self._in[v]));
  }

  sinks() {
    const self = this;
    return lodash.filter(this.nodes(), (v) => lodash.isEmpty(self._out[v]));
  }
  ...
  removeNode(v) {
    ...
    if (lodash.has(this._nodes, v)) {
      ...
      if (this._isCompound) {
        ...
        lodash.each(this.children(v), (child) => {
  ...
  filterNodes(filter) {
    ...
    const self = this;
    lodash.each(this._nodes, (value, v) => {
      if (filter(v)) {
        copy.setNode(v, value);
      }
    });

    lodash.each(this._edgeObjs, (e) => {
  ...
  inEdges(v, u) {
    ...
    if (inV) {
      ...
      return lodash.filter(edges, (edge) => edge.v === u);
    }
  }
  ...
  outEdges(v, w) {
    ...
    if (outV) {
      ...
      return lodash.filter(edges, (edge) => edge.w === w);
    }
  }
...
function edgeObjToId(isDirected, { v, w, name }) {
  ...
  if (lodash.isObject(graphAttrs)) {
    lodash.each(graphAttrs, (v, k) => {
